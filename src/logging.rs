...
fn log_level_icon(level: Level) -> &'static str {
    match level {
        Level::Trace => "",
        Level::Debug => "",
        Level::Info => "",
...
fn log_level_color(level: Level) -> &'static str {
    match level {
        Level::Trace => "#ebdbb2",
        Level::Debug => "#d3869b",
        Level::Info => "#83a598",
...
pub fn setup_logging(verbosity: log::LevelFilter) {
    fern::Dispatch::new()
        ...
        .format(|out, message, record| {
            if record.target() == "tick" {
                return out.finish(format_args!(
                    "<span style=\"color: black; background: {}; font-weight: bold\">[ {} ]</span>",
            ...
            out.finish(format_args!(
                ...
                log_level_color(record.level()),
                log_level_icon(record.level()),
                record.target().replace(env!("CARGO_CRATE_NAME"), "bot"),
                message
            ))
        })
