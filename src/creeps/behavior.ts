...
@profile
class CreepBehavior {
  static getTask(creep: Creep, defaultTask: CreepTask): CreepTask {
  ...
  static harvester(creep: Creep) {
    const task = CreepBehavior.getTask(creep, CreepTask.harvest);
    ...
    switch (task) {
      ...
      case CreepTask.deposit: {
        if (creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
          ...
          CreepBehavior.switchTaskAndDoRoll(creep, CreepTask.harvest);
          return;
        }
        break;
      }
  ...
  /**
   * Behavior static for a miner creep (CreepRole.miner). This creep should
   * move to its assigned spot and mine. If there is an adjacent link, it
   * should deposit energy into the link when possible. When not mining, repair
   * the container at its spot if present.
   *
   * @param creep The miner creep
   */
  static miner(creep: Creep) {
    // Creep is taskless
  ...
  /**
   * Behavior static for builder creeps (CreepRole.builder). These creeps
   * should construct buildings in the build queue.
   *
   * @param creep The builder creep
   */
  static builder(creep: Creep) {
  ...
  static extractor(creep: Creep): void {
    const task = CreepBehavior.getTask(creep, CreepTask.harvest);
    // Tasks for this creep:
  ...
  static claimer(creep: Creep) {
    const task = CreepBehavior.getTask(creep, CreepTask.reserve);
      ...
      case CreepTask.claim: {
        ...
        if (response === ERR_NOT_IN_RANGE) {
          ...
          warn(
            `Creep ${
              creep.name
            } failed to ${action} controller because ${errorConstant(
              response,
            )}`,
          );
        }
  ...
  static remoteHauler(creep: Creep) {
  ...
  static escort(creep: Creep): void {
  ...
  static rangedHarvester(creep: Creep): void {
    const task = CreepBehavior.getTask(creep, CreepTask.getEnergy);
  ...
  static renewCreep(creep: Creep): void {
    ...
    if (creep.ticksToLive && creep.ticksToLive < 1400 && energyRatio > 0.5) {
      ...
      if (response === ERR_NOT_IN_RANGE) {
        ...
        CreepBehavior.switchTaskAndDoRoll(creep, CreepTask.fresh);
      }
    } else {
      CreepBehavior.switchTaskAndDoRoll(creep, CreepTask.fresh);
    }
  ...
  static creepBehavior(creep: Creep): void {
    ...
    try {
      ...
      if (task === CreepTask.renew) {
        CreepBehavior.renewCreep(creep);
    ...
    switch (creepInfo.getRole()) {
      case CreepRole.harvester:
        CreepBehavior.harvester(creep);
        break;
      case CreepRole.builder:
        CreepBehavior.builder(creep);
        break;
      case CreepRole.miner:
        CreepBehavior.miner(creep);
        break;
      case CreepRole.upgrader:
        CreepBehavior.upgrader(creep);
        break;
      case CreepRole.hauler:
        CreepBehavior.hauler(creep);
        break;
      case CreepRole.tender:
        CreepBehavior.tender(creep);
        break;
      case CreepRole.extractor:
        CreepBehavior.extractor(creep);
        break;
      case CreepRole.claimer:
        CreepBehavior.claimer(creep);
        break;
      case CreepRole.remoteHauler:
        CreepBehavior.remoteHauler(creep);
        break;
      ...
      case CreepRole.escort:
        CreepBehavior.escort(creep);
        break;
      case CreepRole.rangedHarvester:
        CreepBehavior.rangedHarvester(creep);
        break;
      default:
